...
export default function App() {
  ...
  const [activeKid, setActiveKid] = useState(() => loggedInKid || KIDS[0].id)
  ...
  // The weekly progress panel shows both kids at once, so it's gated by
  // the parent passcode rather than by whichever kid is "active".
  function requestParentView() {
    ...
    setPendingAction({
      title: 'Parent passcode required',
      message: "See this week's progress for both kids.",
      run: () => {
  ...
  if (!loggedInKid) {
    return <LoginGate kids={KIDS} onLogin={handleLogin} />
  }
